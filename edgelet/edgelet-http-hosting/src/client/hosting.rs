...
//    type Chunk: AsRef<[u8]>;
    ...
    type DeviceConnectionInformationFuture: Future<Item = DeviceConnectionInfo, Error = Self::Error> + Send;
...
//        Box::new(result)
        ...
        let request = self.client.read().expect("RwLock read failure")
            .clone()
...
//                    device_connection_info?
                        ...
                        device_connection_info.ok_or_else(|| {
                            Error::from(ErrorKind::GetDeviceConnectionInformation)
                        })
                },
            );
        Box::new(request)
